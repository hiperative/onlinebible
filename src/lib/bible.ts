...
import { makeBibleLink, truncateText } from './utils'
...
const ENTRY_TYPES = ['book', 'chapter'] as const
...
export const getPaths = async (): Promise<PathEntry[]> => {
  ...
  for await (const book of books) {
    ...
    verses.forEach((verse) => {
      const versesPerChapter = verses.filter((v) => v.chapter === verse.chapter)
      paths.push({
